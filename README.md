...
3. ファイルを編集して、WiFi 接続情報や Windows ファイル共有の情報、利用したいリモートドライブや HDS ファイルの場所を設定します。
    ...
    * TADJUST には、X68000Z 起動時に時刻を Windows 側に合わせる際に取得した時刻から何秒後の時刻を設定するかを指定します。
      * 時刻同期が X68000Z - ラズパイ Pico W - Windows PC 間の通信時間を考慮していない簡易的な実装のためです。
      * ここを空欄にすると起動時の時刻同期を行わないようになります。
      * 現状、X68000Z のエミュレータ側での時刻設定は本体の RTC 時刻に反映されないため、ゲームランチャー等の時刻表示を補正するためにはランチャー側の日時設定機能を使う必要があります。
